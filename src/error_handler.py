...
def validate_forecast_days(forecast_days):
    """
    Validates the forecast_days input. Ensures it is an integer between 1 and 5.
    ...
    try:
        ...
        if forecast_days < 1 or forecast_days > 5:
            return fh.Div(
                fh.P("Error: Please enter a number between 1 and 5 for forecast days.", cls="error-message")
            )
